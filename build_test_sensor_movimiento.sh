...
# =============================================
# 🏃 SCRIPT DE TESTING - SENSOR DE MOVIMIENTO
# =============================================

echo "🏃 COMPILANDO Y EJECUTANDO TESTS DE SENSOR DE MOVIMIENTO"
echo "========================================================"
...
SENSOR_MOV_CPP="lib/Sensores/SensorMovimiento.cpp"
TEST_FILE="test/test_sensor_movimiento_gtest.cpp"
OUTPUT="test_sensor_movimiento_gtest.exe"
...
check_file "$SENSOR_MOV_CPP" || exit 1
...
# Compilar implementación de SensorMovimiento
print_step "Compilando implementación de SensorMovimiento..."
$CXX $CXXFLAGS $INCLUDES -c "$SENSOR_MOV_CPP" -o SensorMovimiento.o
if [ $? -ne 0 ]; then
    echo "❌ Error compilando SensorMovimiento.cpp"
    exit 1
fi
echo "✅ SensorMovimiento.cpp compilado"

# Compilar tests
print_step "Compilando tests..."
$CXX $CXXFLAGS $INCLUDES -c "$TEST_FILE" -o test_sensor_movimiento_gtest.o
if [ $? -ne 0 ]; then
...
$CXX test_sensor_movimiento_gtest.o SensorMovimiento.o mocks.o $LIBS -o "$OUTPUT"
if [ $? -ne 0 ]; then
    echo "❌ Error enlazando"
    echo "Probando enlazado alternativo..."
    # Intentar sin rutas específicas
    $CXX test_sensor_movimiento_gtest.o SensorMovimiento.o mocks.o -lgtest -lgtest_main -o "$OUTPUT"
    if [ $? -ne 0 ]; then
...
echo ""
echo "========================================================"
echo "🧪 EJECUTANDO TESTS DE SENSOR DE MOVIMIENTO"
echo "========================================================"
chmod +x "$OUTPUT"
./"$OUTPUT"
...
if [ $TEST_RESULT -eq 0 ]; then
    ...
    echo "✅ Sensor de Movimiento funcionando correctamente"
else
