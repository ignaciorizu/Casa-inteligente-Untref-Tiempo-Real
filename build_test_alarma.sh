...
# =============================================
# 🏠 SCRIPT DE TESTING - SISTEMA DE ALARMA
# =============================================

echo "🚨 COMPILANDO Y EJECUTANDO TESTS DE ALARMA"
echo "=========================================="
...
ALARMA_CPP="lib/Alarma/Alarma.cpp"
TEST_FILE="test/test_alarma_gtest.cpp"
OUTPUT="test_alarma_gtest.exe"
...
# Compilar implementación de Alarma
print_step "Compilando implementación de Alarma..."
$CXX $CXXFLAGS $INCLUDES -c "$ALARMA_CPP" -o Alarma.o
if [ $? -ne 0 ]; then
    echo "❌ Error compilando Alarma.cpp"
    exit 1
fi
echo "✅ Alarma.cpp compilado"

# Compilar tests
print_step "Compilando tests..."
$CXX $CXXFLAGS $INCLUDES -c "$TEST_FILE" -o test_alarma_gtest.o
if [ $? -ne 0 ]; then
...
$CXX test_alarma_gtest.o Alarma.o mocks.o $LIBS -o "$OUTPUT"
if [ $? -ne 0 ]; then
    echo "❌ Error enlazando"
    echo "Probando enlazado alternativo..."
    # Intentar sin rutas específicas
    $CXX test_alarma_gtest.o Alarma.o mocks.o -lgtest -lgtest_main -o "$OUTPUT"
    if [ $? -ne 0 ]; then
...
if [ $TEST_RESULT -eq 0 ]; then
    ...
    echo "✅ Sistema de Alarma funcionando correctamente"
else
